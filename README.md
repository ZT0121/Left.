...
- 首頁顯示預估月底總結餘、接下來可運用、本期已花、待收與本期收入
- 首頁同時顯示信用卡待繳、本期已繳、下期預估與未來分期未償餘額
- 工具頁可按月份查看歷史收入、支出、結餘與系統自動判斷的記錄類型，不必手動分類
- 首頁下方只顯示最近 5 筆收入、支出與待收動態；待收、信用卡與分期完整明細由摘要或側邊欄進入
- Android PWA 可開啟系統推播：實際卡費到期前 3 天、到期當天仍未繳，以及待收款超過 1 天時提醒
- 打開 App 時同樣會在首頁醒目顯示待處理卡費與逾時待收款
- 一般支出只需要金額、日期與備註
...
- 首頁的「接下來可運用」會從總結餘扣掉最低保留與未來分期承諾；尚未記錄的生活費仍會從這筆金額支出。
